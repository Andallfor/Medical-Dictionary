...
export class Dictionary {
    ...
    static save(file: string) {
        ...
        // feff is utf16-be bom but also doubles as "UTF8 with BOM" marker, which is needed for excel
    ...
    */
    ...
    private static asFormatted(words: Word[]): string {
        ...
        const text: string[][] = words.map(x => {
            let pron = '';

            // internal, always save as is
            if (x.source == StandardType.internal) pron = x.pronunciation?.text ?? '';
            // otherwise, this is an autogenerated word - get its raw pron to save instead (of translated pron)
            // if (x.word in this.debugInternalLookup)
            else if (x.word in this.debugInternalLookup) pron = this.debugInternalLookup[x.word][0];
            else pron = x.pronunciation?.text ?? '';

            return [srcMap[x.source], x.word, pron, x.part, ...x.def];
        });
